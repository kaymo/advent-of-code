#
# Day 6
# Input: Instructions for turning a grid of lights off and on
#
...
# Get the rectangle's corner coordinates from an instruction
def get_rect ( instruction ):

	points = instruction.split(" through ")
	
	coords = {}
	coords['x'] = [0,0]
	coords['y'] = [0,0]
	
	coords['x'][0] = int(points[0].split(',')[0])
	coords['x'][1] = int(points[1].split(',')[0])
	coords['y'][0] = int(points[0].split(',')[1])
	coords['y'][1] = int(points[1].split(',')[1])
	
	return coords
	
# Carry out action depending on whether this is for Part 1 or Part 2
def carry_out_action ( part, action, x, y, grid ):
...
def set_lights ( part ):
	...
	# Start the 1000x1000 grid of lights with all turned off
	grid = []
	for i in xrange(0, 1000):
		grid.append([OFF] * 1000)
	
	for instruction in instructions:
	
		# Parse the action
		if instruction.startswith("turn off "):
			instruction = instruction.split("turn off ")[1]
			action = OFF
		
		elif instruction.startswith("turn on "):
			instruction = instruction.split("turn on ")[1]
			action = ON
		
		elif instruction.startswith("toggle "):
			instruction = instruction.split("toggle ")[1]
			action = TOGGLE
		...
		if action in [OFF, ON, TOGGLE]:
		
			# Parse the (x,y) coordinates of the rectangle corners
			coords = get_rect( instruction )
		
			# Carry out the action for each light within the rectangle
			for x in xrange( coords['x'][0], coords['x'][1] + 1 ):
				for y in xrange( coords['y'][0], coords['y'][1] + 1 ):
					grid = carry_out_action ( part, action, x, y, grid )
	
	# Want the total brightness of lights once all instructions have been carried out
...
if __name__ == "__main__":
	...
	# Part 1
	#
	print "Part 1: Number of lights now lit: {}".format( set_lights ( 1 ) )
	
	
	#
	# Part 2
	#
	print "Part 2: Number of lights now lit: {}".format( set_lights ( 2 ) )
