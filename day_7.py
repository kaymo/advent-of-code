...
def bin_add_16 ( in_1, in_2 ):
    return int(format( in_1, '016b'), 2) & int(format( in_2, '016b'), 2)
    
def bin_or_16 ( in_1, in_2 ):
    return int(format( in_1, '016b'), 2) | int(format( in_2, '016b'), 2)
    
def bin_not_16 ( in_1 ):
    return (1 << 16) - in_1 - 1
    
def bin_ls_16 ( in_1, shift ):
    return in_1 << shift
    
def bin_rs_16 ( in_1, shift ):
    return in_1 >> shift

#
# Check if a wire's signal is known
#
    
def is_known ( values, input ):
...
#
# Run through all wires repeatedly until desired wire's signal is known
#

def get_signal ( signals, wire ):
    
    # Split the signals into 'in' and 'out'
    in_out = []
    
    for signal in signals:
        wires = signal.split(" -> ")
        in_out.append( { "in": wires[0], "out": wires[1] } )
        
    # Limit the while loop to the worst case scenario number of iterations
    max_iters = len(in_out) * ( len(in_out) + 1 ) / 2
    
    # Run through the wires until all 'in' values are known
    done = False
    num_iters = 0
    values = {}
    ...
    while not done and num_iters < max_iters:
        ...
        # Run through each remaining signal, and remove once computed
        for signal in in_out:
        
            # If the input values are known now, compute the output
            if "NOT" in signal["in"]: 
                inputs = signal["in"].split("NOT ")
                
                input_0 = is_known(values, inputs[1])
                
                if not UNKNOWN == input_0:
                    values[signal["out"]] = bin_not_16( input_0 )
                     
            elif "AND" in signal["in"]:
                inputs = signal["in"].split(" AND ")
                
                input_0 = is_known(values, inputs[0])
                input_1 = is_known(values, inputs[1])
                
                if not UNKNOWN == input_0 and not UNKNOWN == input_1:
                    values[signal["out"]] = bin_add_16( input_0, input_1 )
                
            elif "OR" in signal["in"]:
                inputs = signal["in"].split(" OR ")
                
                input_0 = is_known(values, inputs[0])
                input_1 = is_known(values, inputs[1])
                
                if not UNKNOWN == input_0 and not UNKNOWN == input_1:
                    values[signal["out"]] = bin_or_16( input_0, input_1 )
                
            elif "LSHIFT" in signal["in"]:
                inputs = signal["in"].split(" LSHIFT ")
                
                input_0 = is_known(values, inputs[0])
                input_1 = is_known(values, inputs[1])
                
                if not UNKNOWN == input_0 and not UNKNOWN == input_1:
                    values[signal["out"]] = bin_ls_16( input_0, input_1 )
                
            elif "RSHIFT" in signal["in"]:
                inputs = signal["in"].split(" RSHIFT ")
                
                input_0 = is_known(values, inputs[0])
                input_1 = is_known(values, inputs[1])
                
                if not UNKNOWN == input_0 and not UNKNOWN == input_1:
                    values[signal["out"]] = bin_rs_16( input_0, input_1 )
        
            elif signal["in"].isdigit():
                values[signal["out"]] = int(signal["in"])
              
            elif signal["in"] in values:
                values[signal["out"]] = values[signal["in"]]
                
            # If the wire's signal is now known,, remove it from the to-do list
            if signal["out"] in values:
                in_out.remove(signal)
                
        # If there's a value for the wire's signal then we've been successful
        if wire in values:
            done = True
            
    if done:
        print "Wire {}: signal {}".format(wire, values[wire])
        return values[wire]
    else:
        print "Not finished"
        return UNKNOWN
...
if __name__ == "__main__":
    ...
    signal = get_signal( signals, "a" )
    
    #
    # Part Two
    #
    
    # Replace the input to wire B with Part One's signal on wire A
    for i in signals:
        if i[-2:] == " b":
            signals.remove(i)
    signals = [str(signal) + " -> b"] + signals
