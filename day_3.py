#
# Day 3
# Input: The input string contains N, S, E, W directions for Christmas deliveries to a 2D lattice of houses
#
# N = '^' 
# S = 'v'
# E = '>'
...
def calc_houses_visited ( instructions, num_workers ):
	...
	for step in input:
		...
		if (i[worker], j[worker]) not in visited:
			visited.append( (i[worker], j[worker]) )
		
	print "Number of houses visited with {} workers: {}".format( num_workers, len(visited) )
...
if __name__ == "__main__":
    ...
    # One worker delivers presents
    calc_houses_visited( input, 1 )
	
	#
	# Part 2
	#	

    # Two workers deliver presents
    calc_houses_visited( input, 2 )